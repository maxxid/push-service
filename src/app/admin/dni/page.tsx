...
export default function DniPage() {
  ...
  const fetchRecords = () => {
    ...
    fetch(`/api/companies/${companyId}/dni`)
      .then(r => r.json())
  ...
  useEffect(() => {
    // Get companyId from session or from companies API
  ...
  const handleDelete = async (dniId: string) => {
    ...
    await fetch(`/api/companies/${companyId}/dni`, {
      method: "DELETE",
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Gestioná quién puede suscribirse a las notificaciones
          </p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => router.back()} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 font-medium text-sm rounded-xl transition-colors">
            Cancelar
          </button>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 mb-4">
        <p className="text-xs text-slate-400">
          Formato CSV: <code className="bg-slate-800 px-1.5 py-0.5 rounded text-slate-300">nombre,apellido,dni,celular</code>
          <span className="ml-4 text-slate-500">{records.filter(r => r.subscribed).length} registrados de {records.length} totales</span>
        </p>
      </div>

      <input type="text" value={filter} onChange={e => setFilter(e.target.value)}
        placeholder="Buscar por DNI o nombre..." className="w-full max-w-sm px-3 py-2 bg-slate-800 border border-slate-700 rounded-xl text-sm text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 mb-4" />

      {records.length === 0 ? (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-12 text-center">
          <p className="text-slate-400 mb-3">No hay afiliados cargados</p>
          <p className="text-xs text-slate-500">Importá un CSV para comenzar</p>
        </div>
      ) : (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={7} className="px-5 py-12 text-center text-slate-500">Sin resultados</td></tr>
              ) : filtered.map(r => (
