    ...
    <div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 mb-4">
        <p className="text-xs text-slate-400">Formato CSV: <code className="bg-slate-800 px-1.5 py-0.5 rounded text-slate-300">nombre,apellido,dni,celular</code>
          <span className="ml-4 text-slate-500">{records.filter(r => r.subscribed).length} registrados de {records.length} totales</span></p>
      </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
          <table className="w-full text-sm">
            <thead className="border-b border-slate-800"><tr className="text-left text-xs font-medium text-slate-500">
              <th className="px-5 py-3">Nombre</th><th className="px-5 py-3">DNI</th><th className="px-5 py-3 hidden md:table-cell">Celular</th><th className="px-5 py-3">Estado</th>
              <th className="px-5 py-3 hidden lg:table-cell">Fecha suscripción</th><th className="px-5 py-3 hidden lg:table-cell">Dispositivo</th><th className="px-5 py-3 text-right">Acciones</th>
            </tr></thead>
            <tbody>
              {filtered.length === 0 ? <tr><td colSpan={7} className="px-5 py-12 text-center text-slate-500">Sin resultados</td></tr> :
                filtered.map(r => (
                  <tr key={r.id} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                    ...
                    <td className="px-5 py-3 text-right">
                      {r.subscribed && <button onClick={() => handleReset(r.id)} className="text-xs text-amber-400 hover:text-amber-300 mr-3">Reestablecer</button>}
