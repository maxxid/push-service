...
export default function EditLandingPage() {
  ...
  useEffect(() => { fetch(`/api/landing-pages/${params.id}`).then(r => r.json()).then(data => { setLanding(data); setTitle(data.title); setBlocks(Array.isArray(data.content) ? data.content : []); setPublished(data.published) }).finally(() => setLoading(false)) }, [params.id])

  const handleSave = async () => { setSaving(true); await fetch(`/api/landing-pages/${params.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title, content: blocks, published }) }); toast.success("Landing actualizada"); setSaving(false) }
